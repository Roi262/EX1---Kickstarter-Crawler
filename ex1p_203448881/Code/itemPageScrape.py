...
def reset_fields(ID):
    """Resetting the global dictionary that holds the fields before parsing a new card."""
...
class MyHTMLParser(HTMLParser):
    """ HTMLParser is an existing class. We inherit from it and override the methods that are called when
     it finds a starttag (e.g. "<div class=... "),  or a piece of data (any information not inside a tag)
     To communicate between the function we use the lookingFor flag which indicate which field we expect
     to run into next."""
    # Initializing lists
    lsStartTags = list()
    lsEndTags = list()
...
def crawlPage(url, ID):
    """Main function, recieves a url of a webpage and
     its card ID and returns the dictionary of all
